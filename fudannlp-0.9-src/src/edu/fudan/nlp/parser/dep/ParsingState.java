...
public class ParsingState {
	...
	public FeatureVector getFeatures() {
		...
		FeatureVector features = new FeatureVector(featureAlphabet);
		int l = 2;
		int r = 2;
		for (int i = 0; i <= l; i++) {
			...
			if (leftFocus - i < 0) {
				features.addFeature(posFeature + "START"
						+ String.valueOf(i - leftFocus), 1);
				features.addFeature(lexFeature + "START"
						+ String.valueOf(i - leftFocus), 1);
			} else {
				features
						.addFeature(
				...
				features.addFeature(lexFeature
				...
				if (subtrees.get(leftFocus - i).leftChildren.size() == 0) {
					...
					for (int j = 0; j < subtrees.get(leftFocus - i).leftChildren
						...
						int leftChildIndex = subtrees.get(leftFocus - i).leftChildren
								.get(j).root;
						features.addFeature(lcLexFeature
								+ instance.forms[leftChildIndex], 1);
						features.addFeature(lcPosFeature
				...
				if (subtrees.get(leftFocus - i).rightChildren.size() == 0) {
					...
					for (int j = 0; j < subtrees.get(leftFocus - i).rightChildren
						...
						int rightChildIndex = subtrees.get(leftFocus - i).rightChildren
								.get(j).root;
						features.addFeature(rcLexFeature
								+ instance.forms[rightChildIndex], 1);
						features.addFeature(rcPosFeature
		...
		for (int i = 0; i <= r; i++) {
			...
			if (rightFocus + i >= subtrees.size()) {
				features.addFeature(posFeature
						+ "END"
						+ String.valueOf(rightFocus + i - subtrees.size() + 1)
								, 1);
				features.addFeature(lexFeature
						+ "END"
						+ String.valueOf(rightFocus + i - subtrees.size() + 1)
