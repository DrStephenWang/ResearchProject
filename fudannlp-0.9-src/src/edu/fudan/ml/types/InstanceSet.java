...
public class InstanceSet {
	...
	public InstanceSet[] split(int i, int n, InstanceSet set) {
		...
		ne[1] = new InstanceSet(set.pipes,
				set.labelAlphabet);
		ne[1].data = new ArrayList<Instance>(shuffled.data.subList(i * length / n,
				(1 + i) * length / n));
		if (i > 0) {
			ne[0].data = new ArrayList<Instance>(shuffled.data.subList(0, i * length / n));
			for (int j = (1 + i) * length / n; j < length; j++) {
				ne[0].data.add(shuffled.data.get(j));
			}
		} else {
			ne[0].data = new ArrayList<Instance>(shuffled.data.subList((i + 1) * length
					/ n, length));
		}
	...
	public void loadThruStagePipes(SequenceReader reader) throws Exception {
